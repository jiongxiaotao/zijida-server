...
public class ProjectService {
		...
		if(userSession != null){
			...
			if (project == null) {
				...
				if(project.getStatus().equals("0"))
					...
					//查询当前用户是否已对该项目进行过评分
					int doneFlag=scoreDao.queryUserProjectDone(project.getId(), userSession.getUnionid());
					if(doneFlag!=-1){
						result = miniService.newSuccessResponseMap();
	...
	public Map<String, Object> insertProject(String loginCode,Project project) {
		...
		// 主要是判断缓存里还有没有
		if(userSession != null){
			project.setUser_id(userSession.getUnionid());
	...
	public Map<String, Object> deleteProject(String loginCode,long projectId) {
		...
		if(userSession != null){
			...
			if(curProject.getUser_id().equals(userSession.getUnionid())){
				//未发布或已发布未开放评分才可以删除
